...
export const event = (name, options = {}, pixelId) => {
  if (typeof window !== 'undefined') {
    ...
    // Extract eventID for deduplication (must be 4th arg)
    ...
    const standardEvents = [
      ...
      'Subscribe', 'ViewContent', 'ViewCategory', 'PageView'
    ];

    if (standardEvents.includes(name)) {
      window.fbq('track', name, restOptions, eventID ? eventParams : undefined);
    } else {
      window.fbq('trackCustom', name, restOptions, eventID ? eventParams : undefined);
    }
  }
};
